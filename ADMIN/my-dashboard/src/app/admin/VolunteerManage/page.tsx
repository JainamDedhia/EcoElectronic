...
const VolunteerManagement = () => {
  const [volunteers, setVolunteers] = useState<Volunteer[]>([]);
  const [collabVolunteers, setCollabVolunteers] = useState<Volunteer[]>([]);
  const [search, setSearch] = useState("");
  const [filterStatus, setFilterStatus] = useState("All");
  const [sortType, setSortType] = useState("name");
  const [drawerOpen, setDrawerOpen] = useState(false);
  ...
  const fetchVolunteers = async () => {
    try {
      ...
      const volunteersData = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...(doc.data() as Partial<Volunteer>),
        status: (doc.data() as Volunteer).status || "Pending",
        tasks: (doc.data() as Volunteer).tasks || [],
      }));
      setVolunteers(volunteersData);
    } catch (error) {
      console.error("Error fetching volunteers:", error);
  ...
  const handleDeleteCollabVolunteer = async (id: string) => {
    try {
  ...
  const acceptRequest = async () => {
    try {
      ...
      for (const volunteer of newVolunteers) {
        ...
        if (volunteerSnap.exists()) {
          await updateDoc(volunteerRef, volunteer);
        } else {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2">
            <VolunteerOverview volunteers={volunteers} />
            <VolunteerTable volunteers={volunteers} handleDelete={handleDelete} />
            <h2 className="text-xl font-semibold mb-2">Collab Volunteer List</h2>
            <VolunteerTable volunteers={collabVolunteers} handleDelete={handleDeleteCollabVolunteer} />
          </div>
          <div>
            <AddVolunteer refreshData={fetchVolunteers} />
          </div>
